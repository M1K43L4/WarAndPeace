...
from multiprocessing import Pool  # THIS IS FOR CONCURRENCY ------------------
...
if __name__ == "__main__":
    ...
    MY_WORKER_POOL = Pool(16)  # THIS IS FOR CONCURRENCY ------- NUMBER IS NUMBER OF THREADS
    ...
    # COUNTS SINGLES
    NUMBER_OF_TIMES_EACH_CHARACTER_C_OCCURS = MY_WORKER_POOL.map((lambda x: FILE_STRING.count(x)), MY_UNIQUES_TUPLE)
    NUMBER_OF_TIMES_EACH_PAIR_OCCURS = MY_WORKER_POOL.map((lambda x: FILE_STRING.count(x)), MY_PAIRS_TUPLE)  # PAIRS
    NUMBER_OF_TIMES_EACH_TRIPLE_OCCURS = MY_WORKER_POOL.map((lambda x: FILE_STRING.count(x)), MY_TRIPLES_TUPLE)
    # TRIPLES
    ...
    MY_PROBABILITIES_OF_EACH_CHARACTER = MY_WORKER_POOL.map(lambda x: round(x / MY_TOTAL_CHARACTERS, 3),
                                                            NUMBER_OF_TIMES_EACH_CHARACTER_C_OCCURS)
    # SPENT ITERATOR AGAIN
    MY_PROBABILITIES_OF_EACH_PAIR = MY_WORKER_POOL.map(lambda x: round(x / MY_TOTAL_CHARACTERS, 3),
                                                       NUMBER_OF_TIMES_EACH_PAIR_OCCURS)  # SPENT
    MY_PROBABILITIES_OF_EACH_TRIPLE = MY_WORKER_POOL.map(lambda x: round(x / MY_TOTAL_CHARACTERS, 3),
    ...
    NUMBER_OF_TIMES_EACH_PAIR_OCCURS = MY_WORKER_POOL.map((lambda x: FILE_STRING.count(x)), MY_PAIRS_TUPLE)
    MY_PROBABILITIES_OF_EACH_PAIR = MY_WORKER_POOL.map(lambda x: round(x / MY_TOTAL_CHARACTERS, 3),
                                                       NUMBER_OF_TIMES_EACH_PAIR_OCCURS)  # SPENT
